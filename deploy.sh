...
docker build . -t "localhost:5000/order-process-manager:$PROJECT_VERSION"
docker push "localhost:5000/order-process-manager:$PROJECT_VERSION"
...
kubectl rollout status deployment/order-process-manager -n microservices-workshop
